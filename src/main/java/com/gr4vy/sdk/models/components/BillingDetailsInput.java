/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.gr4vy.sdk.models.components;

import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.gr4vy.sdk.utils.Utils;
import java.lang.Override;
import java.lang.String;
import java.lang.SuppressWarnings;
import org.openapitools.jackson.nullable.JsonNullable;

public class BillingDetailsInput {

    /**
     * The first name(s) or given name for the buyer.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("first_name")
    private JsonNullable<String> firstName;

    /**
     * The last name, or family name, of the buyer.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("last_name")
    private JsonNullable<String> lastName;

    /**
     * The email address for the buyer.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("email_address")
    private JsonNullable<String> emailAddress;

    /**
     * The phone number for the buyer which should be formatted according to the E164 number standard.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("phone_number")
    private JsonNullable<String> phoneNumber;

    /**
     * The billing address for the buyer.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("address")
    private JsonNullable<? extends Address> address;

    /**
     * The tax ID information associated with the billing details.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("tax_id")
    private JsonNullable<? extends TaxId> taxId;

    @JsonCreator
    public BillingDetailsInput(
            @JsonProperty("first_name") JsonNullable<String> firstName,
            @JsonProperty("last_name") JsonNullable<String> lastName,
            @JsonProperty("email_address") JsonNullable<String> emailAddress,
            @JsonProperty("phone_number") JsonNullable<String> phoneNumber,
            @JsonProperty("address") JsonNullable<? extends Address> address,
            @JsonProperty("tax_id") JsonNullable<? extends TaxId> taxId) {
        Utils.checkNotNull(firstName, "firstName");
        Utils.checkNotNull(lastName, "lastName");
        Utils.checkNotNull(emailAddress, "emailAddress");
        Utils.checkNotNull(phoneNumber, "phoneNumber");
        Utils.checkNotNull(address, "address");
        Utils.checkNotNull(taxId, "taxId");
        this.firstName = firstName;
        this.lastName = lastName;
        this.emailAddress = emailAddress;
        this.phoneNumber = phoneNumber;
        this.address = address;
        this.taxId = taxId;
    }
    
    public BillingDetailsInput() {
        this(JsonNullable.undefined(), JsonNullable.undefined(), JsonNullable.undefined(), JsonNullable.undefined(), JsonNullable.undefined(), JsonNullable.undefined());
    }

    /**
     * The first name(s) or given name for the buyer.
     */
    @JsonIgnore
    public JsonNullable<String> firstName() {
        return firstName;
    }

    /**
     * The last name, or family name, of the buyer.
     */
    @JsonIgnore
    public JsonNullable<String> lastName() {
        return lastName;
    }

    /**
     * The email address for the buyer.
     */
    @JsonIgnore
    public JsonNullable<String> emailAddress() {
        return emailAddress;
    }

    /**
     * The phone number for the buyer which should be formatted according to the E164 number standard.
     */
    @JsonIgnore
    public JsonNullable<String> phoneNumber() {
        return phoneNumber;
    }

    /**
     * The billing address for the buyer.
     */
    @SuppressWarnings("unchecked")
    @JsonIgnore
    public JsonNullable<Address> address() {
        return (JsonNullable<Address>) address;
    }

    /**
     * The tax ID information associated with the billing details.
     */
    @SuppressWarnings("unchecked")
    @JsonIgnore
    public JsonNullable<TaxId> taxId() {
        return (JsonNullable<TaxId>) taxId;
    }

    public final static Builder builder() {
        return new Builder();
    }    

    /**
     * The first name(s) or given name for the buyer.
     */
    public BillingDetailsInput withFirstName(String firstName) {
        Utils.checkNotNull(firstName, "firstName");
        this.firstName = JsonNullable.of(firstName);
        return this;
    }

    /**
     * The first name(s) or given name for the buyer.
     */
    public BillingDetailsInput withFirstName(JsonNullable<String> firstName) {
        Utils.checkNotNull(firstName, "firstName");
        this.firstName = firstName;
        return this;
    }

    /**
     * The last name, or family name, of the buyer.
     */
    public BillingDetailsInput withLastName(String lastName) {
        Utils.checkNotNull(lastName, "lastName");
        this.lastName = JsonNullable.of(lastName);
        return this;
    }

    /**
     * The last name, or family name, of the buyer.
     */
    public BillingDetailsInput withLastName(JsonNullable<String> lastName) {
        Utils.checkNotNull(lastName, "lastName");
        this.lastName = lastName;
        return this;
    }

    /**
     * The email address for the buyer.
     */
    public BillingDetailsInput withEmailAddress(String emailAddress) {
        Utils.checkNotNull(emailAddress, "emailAddress");
        this.emailAddress = JsonNullable.of(emailAddress);
        return this;
    }

    /**
     * The email address for the buyer.
     */
    public BillingDetailsInput withEmailAddress(JsonNullable<String> emailAddress) {
        Utils.checkNotNull(emailAddress, "emailAddress");
        this.emailAddress = emailAddress;
        return this;
    }

    /**
     * The phone number for the buyer which should be formatted according to the E164 number standard.
     */
    public BillingDetailsInput withPhoneNumber(String phoneNumber) {
        Utils.checkNotNull(phoneNumber, "phoneNumber");
        this.phoneNumber = JsonNullable.of(phoneNumber);
        return this;
    }

    /**
     * The phone number for the buyer which should be formatted according to the E164 number standard.
     */
    public BillingDetailsInput withPhoneNumber(JsonNullable<String> phoneNumber) {
        Utils.checkNotNull(phoneNumber, "phoneNumber");
        this.phoneNumber = phoneNumber;
        return this;
    }

    /**
     * The billing address for the buyer.
     */
    public BillingDetailsInput withAddress(Address address) {
        Utils.checkNotNull(address, "address");
        this.address = JsonNullable.of(address);
        return this;
    }

    /**
     * The billing address for the buyer.
     */
    public BillingDetailsInput withAddress(JsonNullable<? extends Address> address) {
        Utils.checkNotNull(address, "address");
        this.address = address;
        return this;
    }

    /**
     * The tax ID information associated with the billing details.
     */
    public BillingDetailsInput withTaxId(TaxId taxId) {
        Utils.checkNotNull(taxId, "taxId");
        this.taxId = JsonNullable.of(taxId);
        return this;
    }

    /**
     * The tax ID information associated with the billing details.
     */
    public BillingDetailsInput withTaxId(JsonNullable<? extends TaxId> taxId) {
        Utils.checkNotNull(taxId, "taxId");
        this.taxId = taxId;
        return this;
    }

    
    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        BillingDetailsInput other = (BillingDetailsInput) o;
        return 
            Utils.enhancedDeepEquals(this.firstName, other.firstName) &&
            Utils.enhancedDeepEquals(this.lastName, other.lastName) &&
            Utils.enhancedDeepEquals(this.emailAddress, other.emailAddress) &&
            Utils.enhancedDeepEquals(this.phoneNumber, other.phoneNumber) &&
            Utils.enhancedDeepEquals(this.address, other.address) &&
            Utils.enhancedDeepEquals(this.taxId, other.taxId);
    }
    
    @Override
    public int hashCode() {
        return Utils.enhancedHash(
            firstName,
            lastName,
            emailAddress,
            phoneNumber,
            address,
            taxId);
    }
    
    @Override
    public String toString() {
        return Utils.toString(BillingDetailsInput.class,
                "firstName", firstName,
                "lastName", lastName,
                "emailAddress", emailAddress,
                "phoneNumber", phoneNumber,
                "address", address,
                "taxId", taxId);
    }
    
    public final static class Builder {
 
        private JsonNullable<String> firstName = JsonNullable.undefined();
 
        private JsonNullable<String> lastName = JsonNullable.undefined();
 
        private JsonNullable<String> emailAddress = JsonNullable.undefined();
 
        private JsonNullable<String> phoneNumber = JsonNullable.undefined();
 
        private JsonNullable<? extends Address> address = JsonNullable.undefined();
 
        private JsonNullable<? extends TaxId> taxId = JsonNullable.undefined();
        
        private Builder() {
          // force use of static builder() method
        }

        /**
         * The first name(s) or given name for the buyer.
         */
        public Builder firstName(String firstName) {
            Utils.checkNotNull(firstName, "firstName");
            this.firstName = JsonNullable.of(firstName);
            return this;
        }

        /**
         * The first name(s) or given name for the buyer.
         */
        public Builder firstName(JsonNullable<String> firstName) {
            Utils.checkNotNull(firstName, "firstName");
            this.firstName = firstName;
            return this;
        }

        /**
         * The last name, or family name, of the buyer.
         */
        public Builder lastName(String lastName) {
            Utils.checkNotNull(lastName, "lastName");
            this.lastName = JsonNullable.of(lastName);
            return this;
        }

        /**
         * The last name, or family name, of the buyer.
         */
        public Builder lastName(JsonNullable<String> lastName) {
            Utils.checkNotNull(lastName, "lastName");
            this.lastName = lastName;
            return this;
        }

        /**
         * The email address for the buyer.
         */
        public Builder emailAddress(String emailAddress) {
            Utils.checkNotNull(emailAddress, "emailAddress");
            this.emailAddress = JsonNullable.of(emailAddress);
            return this;
        }

        /**
         * The email address for the buyer.
         */
        public Builder emailAddress(JsonNullable<String> emailAddress) {
            Utils.checkNotNull(emailAddress, "emailAddress");
            this.emailAddress = emailAddress;
            return this;
        }

        /**
         * The phone number for the buyer which should be formatted according to the E164 number standard.
         */
        public Builder phoneNumber(String phoneNumber) {
            Utils.checkNotNull(phoneNumber, "phoneNumber");
            this.phoneNumber = JsonNullable.of(phoneNumber);
            return this;
        }

        /**
         * The phone number for the buyer which should be formatted according to the E164 number standard.
         */
        public Builder phoneNumber(JsonNullable<String> phoneNumber) {
            Utils.checkNotNull(phoneNumber, "phoneNumber");
            this.phoneNumber = phoneNumber;
            return this;
        }

        /**
         * The billing address for the buyer.
         */
        public Builder address(Address address) {
            Utils.checkNotNull(address, "address");
            this.address = JsonNullable.of(address);
            return this;
        }

        /**
         * The billing address for the buyer.
         */
        public Builder address(JsonNullable<? extends Address> address) {
            Utils.checkNotNull(address, "address");
            this.address = address;
            return this;
        }

        /**
         * The tax ID information associated with the billing details.
         */
        public Builder taxId(TaxId taxId) {
            Utils.checkNotNull(taxId, "taxId");
            this.taxId = JsonNullable.of(taxId);
            return this;
        }

        /**
         * The tax ID information associated with the billing details.
         */
        public Builder taxId(JsonNullable<? extends TaxId> taxId) {
            Utils.checkNotNull(taxId, "taxId");
            this.taxId = taxId;
            return this;
        }
        
        public BillingDetailsInput build() {
            return new BillingDetailsInput(
                firstName,
                lastName,
                emailAddress,
                phoneNumber,
                address,
                taxId);
        }
    }
}
