...
public class Error405 extends Gr4vyError {
    ...
    /**
    * Parse a response into an instance of Error405. If deserialization of the response body fails,
    * the resulting Error405 instance will have a null data() value and a non-null deserializationException().
    */
    public static Error405 from(HttpResponse<InputStream> response) {
        byte[] bytes;
        try {
            bytes = Utils.extractByteArrayFromBody(response);
        } catch (Exception e) {
            return new Error405(response.statusCode(), null, response, null, e);
        }
        try {
            Data data = Utils.mapper().readValue(bytes, Data.class);
            return new Error405(response.statusCode(), bytes, response, data, null);
        } catch (Exception e) {
            return new Error405(response.statusCode(), bytes, response, null, e);
        }
    }

    /**
    * Parse a response into an instance of Error405 asynchronously. If deserialization of the response body fails,
    * the resulting Error405 instance will have a null data() value and a non-null deserializationException().
    */
    public static CompletableFuture<Error405> fromAsync(HttpResponse<Blob> response) {
